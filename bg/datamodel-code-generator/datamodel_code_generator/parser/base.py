...
def sort_data_models(
    ...
    for model in unsorted_data_models:
        ...
        elif model.path in model.reference_classes and len(model.reference_classes) == 1:
            sorted_data_models[model.path] = model
            require_update_action_models.append(model.path)
        elif not model.reference_classes - {model.path} - set(sorted_data_models):
            sorted_data_models[model.path] = model
...
class Parser(ABC):
    def __init__(
        ...
        *,
        ...
        if self.use_annotated and not self.field_constraints:
        ...
        for model in models:
            ...
            for data_type in model.all_data_types:
                ...
                if from_ and import_ and alias != name:
                    data_type.alias = (
                        alias if data_type.reference.short_name == import_ else f"{alias}.{name}"
                    )
                ...
                for data_type in field.data_type.data_types:
                    ...
                    if mapping:
                        for name, path in mapping.items():
                            if discriminator_model.path.split("#/")[-1] != path.split("#/")[-1]:
                                if (
                                    path.startswith("#/")
                                    or discriminator_model.path[:-1] != path.split("/")[-1]
                                ):
        ...
        for model in models:
            for model_field in model.fields:
                if not model_field.data_type.reference or model_field.has_default:
                    continue
                if isinstance(model_field.data_type.reference.source, DataModel):
            ...
            for model_field in model.fields:
                for data_type in model_field.data_type.all_data_types:
                    ...
                    if (
                        ...
                        and any(d for d in model_field.data_type.all_data_types if d.is_dict or d.is_union)
                    ):
                        continue
                    ...
                    if data_type.reference and isinstance(data_type.reference.source, Enum):
                        if isinstance(model_field.default, list):
                            enum_member: Union[List[Member], Optional[Member]] = [
                                e
                                for e in (
                                    data_type.reference.source.find_member(d) for d in model_field.default
                                )
                                if e
                            ]
                        else:
    ...
    def __set_one_literal_on_default(self, models: List[DataModel]) -> None:
        ...
        for model in models:
            for model_field in model.fields:
                ...
                if model_field.nullable is not True:
