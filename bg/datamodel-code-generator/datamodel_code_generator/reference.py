...
class ModelResolver:
    ...
    def add_ref(self, ref: str, resolved: bool = False) -> Reference:
        ...
        if len(split_ref) == 1:
            original_name = Path(split_ref[0][:-1] if self.is_external_root_ref(path) else split_ref[0]).stem
        else:
            original_name = Path(split_ref[1][:-1]).stem if self.is_external_root_ref(path) else split_ref[1]
