...
EXCLUDE_FIELD_KEYS = (
    set(JsonSchemaObject.get_fields()) - DEFAULT_FIELD_KEYS - EXCLUDE_FIELD_KEYS_IN_JSON_SCHEMA
) | {
    "$id",
    "$ref",
...
class JsonSchemaParser(Parser):
    ...
    def __init__(
        ...
        *,
        ...
        target_python_version: PythonVersion = PythonVersion.PY_312,
    ...
    def is_constraints_field(self, obj: JsonSchemaObject) -> bool:
        return obj.is_array or (
            ...
            and not (obj.ref or obj.anyOf or obj.oneOf or obj.allOf or obj.is_object or obj.enum)
        )
        ...
        for all_of_item in obj.allOf:
            if all_of_item.ref:  
                ...
                if all_of_item.anyOf:
                    ...
                    union_models.extend(
                        d.reference for d in self.parse_any_of(name, all_of_item, path) if d.reference
                    )
                if all_of_item.oneOf:
                    self.model_resolver.add(path, name, class_name=True, loaded=True)
                    union_models.extend(
                        d.reference for d in self.parse_one_of(name, all_of_item, path) if d.reference
                    )
    ...
    def parse_item(
        ...
        elif item.discriminator and parent and parent.is_array and (item.oneOf or item.anyOf):
    ...
    def parse_root_type(
        ...
        if obj.ref:
            ...
            data_type = self.parse_array_fields(name, obj, get_special_path("array", path)).data_type
    ...
    def parse_enum(
        ...
        for i, enum_part in enumerate(enum_times):
            if obj.type == "string" or isinstance(enum_part, str):
                default = (
                    f"'{enum_part.translate(escape_characters)}'" if isinstance(enum_part, str) else enum_part
                )
